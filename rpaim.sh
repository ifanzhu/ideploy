...
MASTER_NGINX_IP=$(getArgValue "$*" "-mip");
#php version
...
restartMasterDomain(){
    expect  $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "kill -HUP \$(ps -ef | grep $MASTER_NGINX | grep 'nginx: master process' | awk '{print \$2}')" ""
    }
...
expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i '/gzip  on;/a\\ upstream  $PHP_APP_DOMAIN{ server  $PHP_APP_INSTANCE_IP:$NGINX_PORT down;  }'  $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" "sed -i '/host.access.log  main;/a\\ location ^~ /$PHP_APP_DOMAIN/ {    proxy_pass   http://$PHP_APP_DOMAIN;  proxy_next_upstream  http_500 http_502 http_503 error timeout invalid_header; proxy_redirect off;  proxy_set_header Host \$host; proxy_set_header X-Real-IP \$remote_addr; proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for; client_body_buffer_size 128k; proxy_connect_timeout 90; proxy_send_timeout 90; proxy_read_timeout 90; proxy_buffer_size 4k; proxy_buffers 4 32k; proxy_busy_buffers_size 64k; proxy_temp_file_write_size 64k;   }' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf"
...
sed  -i "s/server_name  localhost;/server_name  $PHP_APP_DOMAIN;/" $PHP_APP_INSTANCE_PATH/conf/nginx.conf
sed  -i "/access_log  logs\/host.access.log  main;/a\\ location ~ \.php$ {\n    root           webapps;\n    fastcgi_pass   127.0.0.1:9000;\n    fastcgi_index  index.php;\n    fastcgi_param  SCRIPT_FILENAME  \$document_root\$fastcgi_script_name;\n    include        fastcgi_params;\n}" $PHP_APP_INSTANCE_PATH/conf/nginx.conf
expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i 's/upstream  $PHP_APP_DOMAIN{/upstream  $PHP_APP_DOMAIN{  server  $PHP_APP_INSTANCE_IP:$NGINX_PORT down;/' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" ""
#echo "create php app instance successfully";
} 
#deploy php app
deployPhpApp(){
...
expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i ':begin; /upstream  $PHP_APP_DOMAIN{/,/}/ { /}/! { $! { N; b begin }; }; s/upstream  $PHP_APP_DOMAIN{.*}//; };'  $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" "sed -i ':begin; /location ^~ \/$PHP_APP_DOMAIN\/ {/,/}/ { /}/! { $! { N; b begin }; }; s/location ^~ \/$PHP_APP_DOMAIN\/ {.*}//; };'  $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf"
restartMasterDomain;
...
expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i 's/server  $PHP_APP_INSTANCE_IP:$NGINX_PORT;//' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" "sed -i 's/server  $PHP_APP_INSTANCE_IP:$NGINX_PORT down;//' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf"
restartMasterDomain;
...
#	echo "start php app instance begin..."
	for webnginx in $PHP_APP_INSTANCE_ROOT/*
        ...
        then
             $webnginx/$NGINX_VERSION/sbin/nginx;
             expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i 's/server  $PHP_APP_INSTANCE_IP:$NGINX_PORT down;/server  $PHP_APP_INSTANCE_IP:$NGINX_PORT;/' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" ""
             restartMasterDomain;
             break;
        fi
	done
     ...
     do
      if [[ "$PHP_APP_INSTANCE_ROOT/$PHP_APP_INSTANCE_NAME" = "$webnginx" ]];
       then
         ...
         #echo "killing $pid"
         kill -9 $pid;
         expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i 's/server  $PHP_APP_INSTANCE_IP:$NGINX_PORT;/server  $PHP_APP_INSTANCE_IP:$NGINX_PORT down;/' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" ""
         restartMasterDomain;
        break;
      fi
    done
...
-deploy)
deploy;
curl "http://${MASTER_NGINX_IP}/ac/afterDeploy.php?app_name=${PHP_APP_NAME}&op_name=deploy&status=$?"
killUnusedWorkerProcess;
;;
-extend)
extend;
curl  "http://${MASTER_NGINX_IP}/ac/afterDeploy.php?app_name=${PHP_APP_NAME}&op_name=extend&status=$?"
killUnusedWorkerProcess;
;;
-uninstall)
uninstall;
...
-start)
startPhpAppInstance;
curl  "http://${MASTER_NGINX_IP}/ac/afterInstOp.php?app_name=${PHP_APP_NAME}&inst_name=${PHP_APP_INSTANCE_NAME}&op_name=start&status=$?"
killUnusedWorkerProcess;
;;
-restart)
restartPhpAppInstance;
curl   "http://${MASTER_NGINX_IP}/ac/afterInstOp.php?app_name=${PHP_APP_NAME}&inst_name=${PHP_APP_INSTANCE_NAME}&op_name=restart&status=$?"
killUnusedWorkerProcess;
;;
-stop)
stopPhpAppInstance;
curl  "http://${MASTER_NGINX_IP}/ac/afterInstOp.php?app_name=${PHP_APP_NAME}&inst_name=${PHP_APP_INSTANCE_NAME}&op_name=stop&status=$?"
killUnusedWorkerProcess;
;;
-delete)
deletePhpAppInstance;
curl   "http://${MASTER_NGINX_IP}/ac/afterInstOp.php?app_name=${PHP_APP_NAME}&inst_name=${PHP_APP_INSTANCE_NAME}&op_name=delete&status=$?"
killUnusedWorkerProcess;
;;
-help)
echo -e "Usage:\n sh paim.sh -deploy[-extend|-uninstall|-start|restart|-stop|-delete] -path <install root path>  -aname <php app name> -iname <php app instance name>  -pkg <php app package name> -port <php app instance port> -domain <domain name> -ip <ip address of instance> -mip <master ip>";
exit 1;
;;
*)
echo -e "Usage:\n sh paim.sh -deploy[-extend|-uninstall|-start|restart|-stop|-delete] -path <install root path>  -aname <php app name> -iname <php app instance name>  -pkg <php app package name> -port <php app instance port> -domain <domain name> -ip <ip address of instance> -mip <master ip>"
