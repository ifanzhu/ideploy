...
expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i '/gzip  on;/a\\ upstream  $JEE_APP_DOMAIN{ server  $JEE_APP_INSTANCE_IP:$TOMCAT_START_PORT down;  }' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" "sed -i '/host.access.log  main;/a\\ location ^~ /$JEE_APP_DOMAIN/ {   proxy_pass   http://$JEE_APP_DOMAIN;  proxy_next_upstream  http_500 http_502 http_503 error timeout invalid_header; proxy_redirect off;  proxy_set_header Host \$host; proxy_set_header X-Real-IP \$remote_addr; proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for; client_body_buffer_size 128k; proxy_connect_timeout 90; proxy_send_timeout 90; proxy_read_timeout 90; proxy_buffer_size 4k; proxy_buffers 4 32k; proxy_busy_buffers_size 64k; proxy_temp_file_write_size 64k;   }' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf"
...
sed  -i "s/8443/$TOMCAT_REDIRECT_PORT/g"  $JEE_APP_INSTANCE_PATH/conf/server.xml
sed  -i "s/8009/$TOMCAT_AJP_PORT/g"  $JEE_APP_INSTANCE_PATH/conf/server.xml
expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i 's/upstream  $JEE_APP_DOMAIN{/upstream  $JEE_APP_DOMAIN{  server  $JEE_APP_INSTANCE_IP:$TOMCAT_START_PORT down;/' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" ""
#echo "create jee  app instance successfully";
}
...
expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i ':begin; /upstream  $JEE_APP_DOMAIN{/,/}/ { /}/! { $! { N; b begin }; }; s/upstream  $JEE_APP_DOMAIN{.*}//; };'  $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" "sed -i ':begin; /location ^~ \/$JEE_APP_DOMAIN\/ {/,/}/ { /}/! { $! { N; b begin }; }; s/location ^~ \/$JEE_APP_DOMAIN\/ {.*}//; };'  $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf"
restartMasterDomain;
...
expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i 's/server  $JEE_APP_INSTANCE_IP:$TOMCAT_START_PORT;//' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" "sed -i 's/server  $JEE_APP_INSTANCE_IP:$TOMCAT_START_PORT down;//' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" 
restartMasterDomain;
    ...
    for webtomcat in $JEE_APP_INSTANCE_ROOT/*
      do
         if [[ "$JEE_APP_INSTANCE_ROOT/$JEE_APP_INSTANCE_NAME" = "$webtomcat" ]];
           then
              $webtomcat/$TOMCAT_VERSION/bin/startup.sh;
              expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i 's/server  $JEE_APP_INSTANCE_IP:$TOMCAT_START_PORT down;/server  $JEE_APP_INSTANCE_IP:$TOMCAT_START_PORT;/' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" ""
             restartMasterDomain;
              break;
         fi
    done
     ...
     do
        if [[ "$JEE_APP_INSTANCE_ROOT/$JEE_APP_INSTANCE_NAME" = "$webtomcat" ]];
           then
             ...
             #echo "killing $pid"
             kill -9 $pid;
        expect $LNC_ROOT_PATH/rmc.sh "$MASTER_NGINX_IP" "sed -i 's/server  $JEE_APP_INSTANCE_IP:$TOMCAT_START_PORT;/server  $JEE_APP_INSTANCE_IP:$TOMCAT_START_PORT down;/' $MASTER_NGINX_DOMAIN_ROOT/conf/nginx.conf" "";
         restartMasterDomain;
              break;
         fi
   done
...
-deploy)
deploy;
curl "http://${MASTER_NGINX_IP}/ac/afterDeploy.php?app_name=${JEE_APP_NAME}&op_name=deploy&status=$?"
;;
-extend)
extend;
curl  "http://${MASTER_NGINX_IP}/ac/afterDeploy.php?app_name=${JEE_APP_NAME}&op_name=extend&status=$?"
;;
-uninstall)
uninstall;
;;
-start)
startJeeAppInstance;
curl  "http://${MASTER_NGINX_IP}/ac/afterInstOp.php?app_name=${JEE_APP_NAME}&inst_name=${JEE_APP_INSTANCE_NAME}&op_name=start&status=$?"
;;
-restart)
restartJeeAppInstance;
curl  "http://${MASTER_NGINX_IP}/ac/afterInstOp.php?app_name=${JEE_APP_NAME}&inst_name=${JEE_APP_INSTANCE_NAME}&op_name=restart&status=$?"
;;
-stop)
stopJeeAppInstance;
curl  "http://${MASTER_NGINX_IP}/ac/afterInstOp.php?app_name=${JEE_APP_NAME}&inst_name=${JEE_APP_INSTANCE_NAME}&op_name=stop&status=$?"
;;
-delete)
deleteJeeAppInstance;
curl  "http://${MASTER_NGINX_IP}/ac/afterInstOp.php?app_name=${JEE_APP_NAME}&inst_name=${JEE_APP_INSTANCE_NAME}&op_name=delete&status=$?"
;;
*)
